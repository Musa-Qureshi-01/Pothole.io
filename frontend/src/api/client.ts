...
const BACKEND_TIMEOUT_MS = Number(import.meta.env.VITE_PREDICT_TIMEOUT_MS ?? 4500);
...
export async function predict(image: File): Promise<PotholePredictionResponse> {
  ...
  try {
    ...
    console.warn('Backend unavailable, using vision fallback.', err);
  }

  // --- OpenRouter vision fallback (for deployments where the local model API is offline) ---
  ...
  if (!response.ok) {
    throw new Error(`Detection fallback failed with status ${response.status}.`);
  }
  ...
  return {
    is_pothole: Boolean(aiResult?.is_pothole),
    ...
    overlay_png_base64: base64Image // Use original image as overlay
  };
}
